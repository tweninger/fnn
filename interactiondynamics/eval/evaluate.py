...
@torch.no_grad()
def evaluate_physical_force_rollout(
    model,
    bins: Iterable[EventBatch],
    cfg,
    *,
    ...
    The model receives one observed starting bin.  At each future step it
    predicts forces for the next bin's source/destination *queries*, consumes
    those predicted-force events, and is compared with the true internal force
    bin. Observed external interventions are injected at their true value.
    This measures propagation error without pretending that source counts or
    measurement queries have already been generated by the model.
    """
    model.eval()
    force_predictor = getattr(model, "predict_event_features", None)
    if not callable(force_predictor):
        return {"rollout_horizon": float(horizon), "rollout_steps": 0.0}
    device = torch.device(cfg.device)
    events_seq = [events.to(device) for events in bins]
    horizon = max(1, int(horizon))
    if len(events_seq) <= horizon:
        return {"rollout_horizon": float(horizon), "rollout_steps": 0.0}
    ...
    # intervention to reveal the physical response to each new drop.
    steps_since_external: list[Optional[int]] = []
    ...
    for events in events_seq:
        ...
        if episode != previous_episode:
            elapsed = None
        has_external = events.is_external is not None and bool(events.is_external.any())
        if has_external:
            elapsed = 0
        elif elapsed is not None:
            elapsed += 1
        steps_since_external.append(elapsed)
        previous_episode = episode
    ...
    for start_idx in tqdm(
        ...
        # endogenous forces nor later observed raindrops update its state.
        persistent_rollout_state = rollout_state.clone(detach=True)
        valid_window = True
        for relative_step, target_idx in enumerate(target_indices, start=1):
            target = events_seq[target_idx]
            if target.features is None:
                valid_window = False
                break
            # Pair features are deliberately absent from the query: force is
            # predicted from the current latent state and pair identity only.
            query = EventBatch(
                src=target.src,
                dst=target.dst,
                t=target.t,
            ...
            predicted_force = force_predictor(rollout_state, query)
            persistent_predicted_force = force_predictor(persistent_rollout_state, query)
            internal = internal_events(target)
            if internal.num_events:
                keep = ~target.is_external.to(dtype=torch.bool) if target.is_external is not None else torch.ones(
                    target.num_events, device=device, dtype=torch.bool
                )
            ...
            if target.is_external is not None and bool(target.is_external.any()):
                ...
                observed_external_events += int(external.sum().item())
            ...
            rollout_state, _ = model.step(rollout_state, generated)
            if rollout_state is None:
                valid_window = False
                break
    ...
    *,
    ...
    for events in tqdm(bins, total=len(bins), desc=progress_desc, unit="bin", leave=False):
        ...
        episode_changed = (
            ...
            and int(prev.episode[0].item()) != int(events.episode[0].item())
        )
        if prev is None or episode_changed:
            if episode_changed:
                state = model.init_state(batch_size=1, num_nodes=cfg.num_nodes, device=device)
                persistent_state = None
                prev_node_target = prev_prev_node_target = prev_prev_prev_node_target = None
                prev_edge_target = prev_prev_edge_target = prev_prev_prev_edge_target = None
            prev = events
