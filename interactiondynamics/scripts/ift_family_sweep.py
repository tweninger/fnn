...
def main():
    ...
    runs = make_runs(
        ...
        ift_dt=(0.01, 0.05, 0.1, 0.2), #just changed these 3
    ...
    args.results_dir, f"{dataset_name}_results_{args.epochs}ep_1seed.jsonl"
    )
    summary_jsonl = os.path.join(
    args.results_dir, f"{dataset_name}_summary_{args.epochs}ep_1seed.jsonl"
    )

    results = [] # don't forget this
